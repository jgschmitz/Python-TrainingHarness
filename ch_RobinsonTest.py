...
def print_test_header():
    print("\n" + "=" * 92)
    print("C.H. ROBINSON EMBEDDING EVALUATION")
    print("=" * 92)
    print(
        f"Queries: {len(TEST_QUERIES)}  |  "
        f"Search: {'Exact' if EXACT_SEARCH else 'ANN'}  |  "
        f"Dimensions: {DIMENSIONS}  |  Top K: {TOP_K}"
    )
    print("-" * 92)
    print(
        f"{'#':>2}  "
        f"{'Expected document':<38}  "
        f"{'Voyage':<12}  "
        f"{'OpenAI':<12}"
    )
    print("-" * 92)
...
def print_query_line(query_number, expected_title, results_by_model):
    voyage = results_by_model["voyage-4-lite"]
    openai_result = results_by_model["text-embedding-3-small"]

    voyage_status = (
        f"{rank_text(voyage['expected_rank']):<5} "
        f"{'OK' if voyage['expected_rank'] == 1 else 'CHECK'}"
    )

    openai_status = (
        f"{rank_text(openai_result['expected_rank']):<5} "
        f"{'OK' if openai_result['expected_rank'] == 1 else 'CHECK'}"
    )

    print(
        f"{query_number:>2}  "
        f"{shorten(expected_title, 38):<38}  "
        f"{voyage_status:<12}  "
        f"{openai_status:<12}"
    )


def print_quality_summary(summary_frame):
    print("\n" + "=" * 92)
    ...
    for _, row in summary_frame.iterrows():
        print(
            f"{row['model']:<28} "
            f"{row['hit_at_1'] * 100:>9.1f}% "
            f"{row['hit_at_3'] * 100:>9.1f}% "
            f"{row['hit_at_5'] * 100:>9.1f}% "
            f"{row['mrr']:>10.3f}"
        )


def print_latency_summary(summary_frame):
    print("\n" + "=" * 92)
    ...
    for _, row in summary_frame.iterrows():
        print(
            f"{row['model']:<28} "
            f"{row['embedding_median_ms']:>10.1f}ms "
            f"{row['embedding_p95_ms']:>10.1f}ms "
            f"{row['search_median_ms']:>10.1f}ms "
            f"{row['search_p95_ms']:>10.1f}ms "
            f"{row['total_median_ms']:>10.1f}ms "
            f"{row['total_p95_ms']:>10.1f}ms"
        )


def print_miss_summary(query_scores_frame):
    print("\n" + "=" * 92)
    print("TOP-1 MISSES")
    print("=" * 92)
    ...
    for config in MODELS:
        ...
        misses = query_scores_frame[
            (query_scores_frame["model"] == model_name)
            & (query_scores_frame["hit_at_1"] == 0)
        ]

        print(f"\n{model_name}: {len(misses)}")
        ...
        for _, row in misses.iterrows():
            print(
                f"  Query    : {row['query']}\n"
                f"  Expected : {row['expected_title']}\n"
                f"  Top 1    : {row['top1_title']}\n"
                f"  Correct  : {rank_text(row['expected_rank'])}\n"
            )

    if not any_misses:
        print("\nBoth models returned the expected document at rank 1 for every query.")


def print_winner(summary_frame):
    ranked = summary_frame.sort_values(
        by=["hit_at_1", "mrr", "total_median_ms"],
    ...
    hit1_difference = (
        winner["hit_at_1"] - runner_up["hit_at_1"]
    ) * 100

    latency_difference = (
        winner["total_median_ms"]
        - runner_up["total_median_ms"]
    )
    ...
    print("=" * 92)
    print(
        f"Retrieval winner: {winner['model']} "
        f"({winner['hit_at_1'] * 100:.1f}% Hit@1, "
        f"MRR {winner['mrr']:.3f})"
    )

    if hit1_difference > 0:
        print(
            f"Top-result accuracy advantage: "
            f"{hit1_difference:.1f} percentage points."
        )

    if latency_difference > 0:
        print(
            f"Median total latency tradeoff: "
            f"{latency_difference:.1f} ms slower."
        )
    elif latency_difference < 0:
        print(
            f"Median total latency advantage: "
            f"{abs(latency_difference):.1f} ms faster."
        )
...
def main():
    ...
    for query_number, test in enumerate(
        ...
        for config in model_order:
            query_vector, embedding_ms = embed_query(
                config,
                query,
                voyage_client,
            )

            documents, search_ms = run_vector_search(
                collection,
                config,
                query_vector,
            )
            ...
            top1_title = (
                str(documents[0].get("title", "")).strip()
                if documents
                else "NO RESULTS"
            )
            ...
            results_by_model[config["label"]] = model_result
    ...
    query_scores_frame = pd.DataFrame(query_score_rows)
    ...
    print_quality_summary(summary_frame)
    print_latency_summary(summary_frame)

    if SHOW_MISS_DETAILS:
        print_miss_summary(query_scores_frame)

    print_winner(summary_frame)
